...
export default async function ProductDetails({ params }: { params: { product: string } }) {
  ...
  alt={product.name}  // Change this to product.productName
  width={400}
  height={400}
...
/>
          ...
          <span className="flex items-center">
            <svg
              ...
              strokeWidth={2}
              className="w-4 h-4 text-custom5b"
              viewBox="0 0 24 24"
            >
          ...
          <div className="flex">
            ...
            <button className="border-2 border-gray-300 ml-1 bg-custom5b rounded-full w-6 h-6 focus:outline-none" />
          </div>
  ...
  <button className="mt-6 bg-custom5b text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-600 transition">
     <Link href={"/buynow"}>
     Buy Now
</Link>
  </button> 
  
  <button className="mt-6 bg-custom5b ml-6 text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-600 transition">
     <Link href={"/wishlist"}>
     ❤️
</Link>
  </button> 
